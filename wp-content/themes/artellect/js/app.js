      ...
      levels.forEach(level => {
         ...
         btn.addEventListener('click', () => {
            ...
            const image = card.querySelector(
               `.art-work__card-image[data-level="${levelValue}"]`
            );
...
window.addEventListener('DOMContentLoaded', function () {
   [].forEach.call(
      document.querySelectorAll('input.tel-mask'),
      function (input) {
         var keyCode;
         function mask(event) {
      ...
      dropdowns.forEach(dropdown => {
         ...
         if (button && list) {
            ...
            button.addEventListener('click', e => {
               ...
               dropdowns.forEach(d => {
                  if (d !== dropdown) {
                     d.querySelector('.theme-dropdown__button')?.classList.remove(
                        'active'
                     );
                     d.querySelector('.theme-dropdown__list')?.classList.remove(
                        'show'
                     );
                  }
               });
      ...
      document.addEventListener('click', e => {
         dropdowns.forEach(dropdown => {
            if (!dropdown.contains(e.target)) {
               dropdown
                  .querySelector('.theme-dropdown__button')
                  ?.classList.remove('active');
               dropdown
                  .querySelector('.theme-dropdown__list')
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   const selectedContainer = document.querySelector(
      '.art-filters__selected-items'
   );
   const resetButton = document.querySelector(
      '.art-filters__reset button[type=reset]'
   );
   ...
   function syncCustomUIAfterUncheck(input) {
      ...
      const box =
         lbl?.querySelector('.theme-dropdown__box') ||
   ...
   function createSelectedItem(input) {
      ...
      item.innerHTML = `<span class="art-filters__selected-text">${name}</span><span class="art-filters__selected-icon"aria-hidden="true"></span>`;
      ...
      if (input.checked) {
         let existingItem = [...selectedContainer.children].find(
            item =>
               ...
               (input.dataset.name || input.value || 'Выбрано')
         );
         if (existingItem) {
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   if (window.location.pathname === '/cases/emercon') {
      ...
      tabs.forEach(tab => {
         tab.addEventListener('click', () => {
            ...
            const activeBlock = document.querySelector(
               `.ads-calculator__block[data-calc-block="${target}"]`
            );
   ...
   if (slider && marioPerson && coins.length) {
      ...
      if (window.innerWidth >= 1000) {
         ...
         function animate() {
            ...
            marioPerson.style.transform =
               targetX > marioX ? 'scaleX(1)' : 'scaleX(-1)';
            coins.forEach(coin => {
      ...
      if (window.innerWidth > 1200 && screenshots) {
         ...
         values.forEach(v => {
            const parentBlock = document.querySelector(
               `.ads-preview__block.ads-preview__key[data-id="${v.dataset.id}"]`
            )?.parentElement;
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   function moveValues() {
      if (window.innerWidth > 1200 && screenshots) {
         ...
         values.forEach(v => {
            const parentBlock = document.querySelector(
               `.ya-stages__block.ya-stages__key[data-id="${v.dataset.id}"]`
            )?.parentElement;
            if (parentBlock && !parentBlock.contains(v)) {
               parentBlock.appendChild(v);
            }
...
function initSwiper(selector, options, fractionSelector = null) {
   ...
   const fraction = fractionSelector
      ? document.querySelector(fractionSelector)
      : null;
...
initSwiper('.art-team__slider', {
   ...
   loop: false,
   watchOverflow: true,
   pagination: { el: '.art-team__slider-pagination', clickable: false },
   navigation: {
...
initSwiper('.art-cases__slider', {
   ...
   loop: false,
   watchOverflow: true,
   pagination: { el: '.art-cases__slider-pagination', clickable: false },
   navigation: {
...
initSwiper('.art-actuals__slider', {
   ...
   loop: false,
   watchOverflow: true,
   pagination: { el: '.art-actuals__pagination', clickable: false },
   navigation: { nextEl: '.art-actuals__next', prevEl: '.art-actuals__prev' },
});
initSwiper('.more-articles__slider', {
   slidesPerView: 'auto',
   loop: false,
   watchOverflow: true,
   pagination: { el: '.more-articles__slider-pagination', clickable: false },
   navigation: {
...
initSwiper('.em-callcenter__slider', {
   ...
   pagination: { el: '.em-callcenter__slider-pagination' },
});
...
analysBlocks.forEach(block => {
   ...
   const swiper = new Swiper(slider, {
      ...
      pagination: { el: block.querySelector('.em-analysis__slider-pagination') },
   });
   ...
   if (fullscreenBtn) {
      fullscreenBtn.addEventListener('click', () => {
         ...
         Fancybox.show(
            ...
            { ...fancyOptions, startIndex: currentIndex }
         );
      });
   }
});
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   if (starsContainer && starSrc) {
      function createStars() {
         const starCount = Math.floor(Math.random() * 2) + 3;
   ...
   if (paths.length > 0) {
      ...
      function animatePath(index) {
         ...
         const overlap = duration * (Math.random() * 0.2 + 0.5);
...
document.addEventListener('DOMContentLoaded', () => {
   const items = document.querySelectorAll('.main_faq_screen_block_list_item');

   items.forEach(item => {
      const question = item.querySelector('.main_faq_screen_block_list_item_q');
      const answer = item.querySelector('.main_faq_screen_block_list_item_a');

      question.addEventListener('click', () => {
         const isOpen = item.classList.contains('main_faq_screen_block_list_item_sel');
         items.forEach(i => {
            i.classList.remove('main_faq_screen_block_list_item_sel');
            i.querySelector('.main_faq_screen_block_list_item_a').style.display = 'none';
         });

         if (!isOpen) {
            item.classList.add('main_faq_screen_block_list_item_sel');
